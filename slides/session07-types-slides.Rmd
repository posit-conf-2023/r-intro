...
???
Suppose we are interested in the bike traffic around labor day (in the year 2015). We can start by creating a date object for our date of interest. 
...
date_interval <- 
  interval(labor_day - weeks(1), labor_day + weeks(1))
...
???
And finally we can filter our dataset to that time interval using an operator called within, which is part of the lubridate package. 
---
# Filtered Result
...
```
???
And here is the filtered result, which is now subset of the original data containing only the records with a timestamp within one week of labor day. 
